...
try:
    ...
    logger.info("✅ MySQL 連接池初始化成功 (Size=20)")
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ 檢測到無效連接，正在丟棄: {ping_err}")
                if conn:
                    try: conn.close() 
                    except: pass
                conn = None 
            ...
            if conn and conn.is_connected():
                if attempt > 0:
                    logger.info(f"✅ 重試後獲取連接成功 (嘗試 {attempt + 1}/{max_retries})")
                return conn
            else:
                if conn: 
                    try: conn.close() 
                    except: pass
                raise MySQLError("連接狀態異常")
    ...
    try:
        ...
        if mysql_conn:
            try:
                ...
                mysql_conn.commit()
                logger.info(f"✅ 【MySQL 寫入成功】 Node {node_id} | temp={temp} | hum={hum}")
                
            except MySQLError as db_err:
                logger.error(f"❌ MySQL 寫入失敗: {db_err}")
                try:
                    mysql_conn.rollback()
            ...
            logger.warning("⚠️ MySQL 未連接，跳過寫入（Redis 已保存）")
...
def get_history_data():
    ...
    try:
        ...
        logger.info(f"✅ 查詢成功: {node_id} | {len(data)} 筆記錄")
        ...
        logger.error(f"❌ history API 錯誤: {e}")
        return jsonify({"code": 500, "msg": str(e), "data": None}), 500
    finally:
        if cursor:
            try:
    ...
    if request.method == 'POST':
        try:
            ...
            config_data = {
                ...
                "mqtt_user": mqtt_user, "mqtt_pass": mqtt_pass, "mqtt_topic": mqtt_topic
            }
